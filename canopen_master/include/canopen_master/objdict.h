...
public:
    ...
    typedef boost::shared_ptr<const Entry> EntrySharedConstPtr;
    ...
    const EntrySharedConstPtr& get(const Key &k) const{
        return at(k);
    }
    ...
    bool insert(bool is_sub, EntrySharedConstPtr e){
        return dict_.insert(std::make_pair(is_sub?Key(e->index,e->sub_index):Key(e->index),e)).second;
    }
    bool iterate(boost::unordered_map<Key, EntrySharedConstPtr >::const_iterator &it) const;
...
protected:
    const EntrySharedConstPtr& at(const Key &key) const{
        try{
            return dict_.at(key);
        }
    ...
    boost::unordered_map<Key, EntrySharedConstPtr > dict_;
};
...
protected:
    ...
    public:
        ...
        const ObjectDict::EntrySharedConstPtr entry;
        const ObjectDict::Key key;
        size_t size() { boost::mutex::scoped_lock lock(mutex); return buffer.size(); }

        template<typename T> Data(const ObjectDict::Key &k, const ObjectDict::EntrySharedConstPtr &e, const T &val, const ReadDelegate &r, const WriteDelegate &w)
        ...
        Data(const ObjectDict::Key &k, const ObjectDict::EntrySharedConstPtr &e, const TypeGuard &t, const ReadDelegate &r, const WriteDelegate &w)
...
protected:
    ...
    void init_nolock(const ObjectDict::Key &key, const ObjectDict::EntrySharedConstPtr &entry);

    ReadDelegate read_delegate_;
    WriteDelegate write_delegate_;
    size_t map(const ObjectDict::EntrySharedConstPtr &e, const ObjectDict::Key &key, const ReadDelegate & read_delegate, const WriteDelegate & write_delegate);
public:
    template<typename T> Entry<T> entry(const ObjectDict::Key &key){
        ...
        if(it == storage_.end()){
            const ObjectDict::EntrySharedConstPtr e = dict_->get(key);
            ...
            it = storage_.insert(std::make_pair(key, data)).first;
        }
    ...
    typedef boost::shared_ptr<const ObjectDict> ObjectDictSharedConstPtr;
    const ObjectDictSharedConstPtr dict_;
    const uint8_t node_id_;

    ObjectStorage(ObjectDictSharedConstPtr dict, uint8_t node_id, ReadDelegate read_delegate, WriteDelegate write_delegate);
