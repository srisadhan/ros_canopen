...
bool ObjectDict::iterate(boost::unordered_map<Key, EntrySharedConstPtr >::const_iterator &it) const{
    if(it != boost::unordered_map<Key, EntrySharedConstPtr >::const_iterator()){
        ++it;
    }else it = dict_.begin();
    return it != dict_.end();
}
...
void parse_object(ObjectDict::ObjectDictSharedPtr dict, boost::property_tree::iptree &pt, const std::string &name, const uint8_t* sub_index = 0){
...
void parse_objects(ObjectDict::ObjectDictSharedPtr dict, boost::property_tree::iptree &pt, const std::string &key){
...
ObjectDict::ObjectDictSharedPtr ObjectDict::fromFile(const std::string &path, const ObjectDict::Overlay &overlay){
    DeviceInfo info;
    boost::property_tree::iptree pt;
    ObjectDict::ObjectDictSharedPtr dict;
...
size_t ObjectStorage::map(const ObjectDict::EntrySharedConstPtr &e, const ObjectDict::Key &key, const ReadDelegate & read_delegate, const WriteDelegate & write_delegate){
...
ObjectStorage::ObjectStorage(ObjectDictSharedConstPtr dict, uint8_t node_id, ReadDelegate read_delegate, WriteDelegate write_delegate)
...
void ObjectStorage::init_nolock(const ObjectDict::Key &key, const ObjectDict::EntrySharedConstPtr &entry){
...
void ObjectStorage::init_all(){
    ...
    boost::unordered_map<ObjectDict::Key, ObjectDict::EntrySharedConstPtr >::const_iterator entry_it;
